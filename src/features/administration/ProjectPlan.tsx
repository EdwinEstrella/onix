import { CalendarDays, CheckCircle2, Circle, Database, GitBranch, ShieldCheck, Sparkles, Stethoscope } from "lucide-react";
...
const phases = [
  ...
  { name: "Fase 3 - Backend", status: "Pendiente", progress: 0, items: ["API autenticada", "PostgreSQL", "Autorizacion por rol", "Reportes reales"] },
              ...
              <div key={phase.name} className="rounded-lg border p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <div className={`mt-0.5 w-8 h-8 rounded-full flex items-center justify-center ${index === 0 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>
                      {index === 0 ? <CheckCircle2 className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
                    </div>
                    <div>
